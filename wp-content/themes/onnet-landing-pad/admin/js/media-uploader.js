jQuery(document).ready(function ($) {
	...
	function optionsframework_add_file_multi(event, selector) {

		var upload = $(".multi-upload-button"), frame;
		var $el = $(this);
		optionsframework_selector = selector;
		...
		if (optionsframework_upload) {
			...
			optionsframework_upload.on('select', function () {
				// Grab the selected attachment.
				var attachments = optionsframework_upload.state().get('selection'),
					optionsframework_uploaded_container = optionsframework_selector.find('.screenshot');

				optionsframework_upload.close();

//				optionsframework_selector.find('.uploads').val(attachment.attributes.url);

				if (undefined !== attachments.models && attachments.models.length > 0) {
					// Empty this div and hide it.
					optionsframework_uploaded_container.empty().hide();

					// Insert all the images
					jQuery.each(attachments.models, function (i, e) {
//						console.log(value);
						if(e.attributes.type === 'image')
						{
							optionsframework_uploaded_container.append('<img src="' + e.attributes.url + '"><a class="remove-image">Remove</a>');
						}
					});

					// Show again
					optionsframework_uploaded_container.slideDown('fast');
				}
...
//				});
	...
	$('.multi-upload-button').click(function (event) {
		optionsframework_add_file_multi(event, $(this).parents('.section'));
	});
